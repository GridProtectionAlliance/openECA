...
var MappingsCtrl = userDefinedMappings.controller("MappingsCtrl",
    function($scope, $location) {
        ...
        $scope.buildDeviceList = function() {
            ...
            $.each(deviceData,
                function(key, device) {
                    ...
                    const index = parents.indexOf(device.ParentAcronym);
            ...
            $.each(measurementDetails,
                function(key, measurement) {
                    ...
                    try {
                        ...
                        const index2 = $scope.deviceList[index].Devices.findIndex(function(elm, i) { return elm.Acronym == measurement.DeviceAcronym });
            ...
            dataHub.getDefinedTypes().done(function(types) {
                ...
                dataHub.getMappingCompilerErrors(output).done(function(data) {
                    ...
                    if (errorList.length == 0) {
                        ...
                        const anchor = $('<a href="#" id="error-count">');
        ...
        $scope.getExpressions = function(item) {
            ...
            $.each(item.FieldMappings,
                function(i, fieldMapping) {
        ...
        $scope.addToExpression = function(x, thecontrol) {
            var string = "";
            $.each(thecontrol.multiselectData,
                function(i, d) {
                    string += d + ";";
                });
        ...
        $scope.addNewSignals = function() {
            ...
            $(promises).whenAll().done(function() {
                ...
                setTimeout(function() {
                        $(window).one("metaDataReceived",
                            function() {
                                dataHub.getMeasurementDetails().done(function(md) {
                                    $.each($scope.mapping.Type.Fields,
                                        function(i, f) {
                                            const signals = $.grep(md, function(d) { return d.PointTag == $("#point" + f.Identifier).val() });
                                            if (signals.length > 0) {
        ...
        $scope.openTimeWindow = function(data) {
            $("#timeWindowSave").off("click").on("click",
                function() {
        ...
        $scope.openTimeWindow2 = function(data) {
            $("#timeWindowSave2").off("click").on("click",
                function() {
                    data.TimeWindowExpression = angular.copy($scope.timeWindowText);
                    $scope.$apply();
                    $scope.clearFields();
        ...
        $scope.updateTimeWindowText = function() {
            ...
            $.each($("#secondMaprt").children().children(),
                function(i, object) {
                    if ($(object).is("select"))
        ...
        $scope.updateTimeWindowText2 = function() {
            ...
            $.each($("#secondMaptw").children().children(),
                function(i, object) {
                    if ($(object).is("select"))
                        $scope.timeWindowText += $(object).children(":selected").text() + " ";
                    else if ($(object).is("input"))
        ...
        $scope.addMultipleSignals = function(x) {
            ...
            $("#addSignalsButton").off("click").on("click",
                function() {
                    x.Expression = "";
                    $("#deviceList input[type=checkbox]").each(function() {
            ...
            $.each(errorList,
                function(key, error) {
                    $("#error-" + key).click(function(e) {
                        ...
                        $("#save-errors").off("click.errors").on("click.errors",
                            function(e) {
                                const contents = $("#input-errors").val();

                                $("#save-errors").off("click.Errors").attr("disabled", "disabled");

...
namespace ConfigurationSetupUtility.Screens
{
    ...
    public partial class ConfigurationTypeScreen : UserControl, IScreen
    {
        ...
        public ConfigurationTypeScreen()
        {
            ...
            try
            {
                // Determine if the openECA is installed in this folder (could be stand-alone debug application or service), we check exe
                // instead of config file since config file can remain between installations. Since only the openECA can be setup with a
                // XML or web service configuration, we disable these radio buttons for openECA Manager only installations.
                string exeFileName = Directory.GetCurrentDirectory() + "\\openECA.exe";

                if (!File.Exists(exeFileName))
                {
                    // If the openECA is not be installed user may have chosen to only install the openECA Manager on this system,
            ...
            {
                // Not failing if we cannot determine if openECA is available...
            }
        }

...
namespace ConfigurationSetupUtility.Screens
{
    ...
    public partial class NodeSelectionScreen : UserControl, IScreen
    {
        ...
        private void UpdateDataGrid()
        {
            ...
            {
                m_infoTextBlock.Text = "Please select the node you would like the openECA to use.";
            }
            else
            {
            ...
            if (applyChangesToService)
                nodeId = GetNodeIdFromConfigFile("openECA.exe.config");
            else if (applyChangesToLocalManager)
                nodeId = GetNodeIdFromConfigFile("openECAManager.exe.config");
            else
                nodeId = Guid.Empty;
            ...
            if (applyChangesToService)
                TryAddNodeInfo("openECA.exe.config");

            if (applyChangesToLocalManager)
                TryAddNodeInfo("openECAManager.exe.config");
        ...
        private IDbConnection GetSqliteDatabaseConnection()
        {
            ...
            string dataProviderString = "AssemblyName={System.Data.SQLite, Version=1.0.108.0, Culture=neutral, PublicKeyToken=db937bc2d44ff139}; ConnectionType=System.Data.SQLite.SQLiteConnection; AdapterType=System.Data.SQLite.SQLiteDataAdapter";
            return GetConnection(connectionString, dataProviderString);
        }
        ...
        private IDbConnection GetConnectionFromConfigFile()
        {
            ...
            string configFileName = Directory.GetCurrentDirectory() + "\\openECA.exe.config";
        ...
        {
            return GetValueOfSystemSetting("openECA.exe.config", "CompanyName");
        }
        ...
        // Occurs when the user selects a different node to be used by the openECA.

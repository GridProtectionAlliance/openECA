...
namespace ConfigurationSetupUtility.Screens
{
    ...
    public partial class SetupCompleteScreen : UserControl, IScreen
    {
        ...
        private ServiceController m_openECAServiceController;
        ...
        {
            InitializeComponent();
            InitializeopenECAServiceController();
            ...
            get
            {
                ...
                if (m_state != null)
                {
                    try
                    {
                        ...
                        if (migrate)
                        {
                            // Convert node table to new format (i.e., columns to connection settings string), if needed
                        ...
                        try
                        {
                            ConfigurationFile openECAConfig = ConfigurationFile.Open("openECA.exe.config");

                            // Some of the crypto settings elements were renamed for consistency, remove the old ones
                            CategorizedSettingsElementCollection cryptoSection = openECAConfig.Settings["cryptographyServices"];
                            cryptoSection.Remove("RetryDelayInterval");
                            cryptoSection.Remove("MaximumRetryAttempts");

                            // Example connection settings are updated between builds - remove the section and openECA will add back the latest
                            openECAConfig.Settings.Remove("exampleConnectionSettings");

                            // Data publisher categories are now always lower case (such that code references are case insensitive)
                            openECAConfig.Settings.Remove("dataPublisher");

                            openECAConfig.Save(ConfigurationSaveMode.Full);
                        }
                        catch
                        {
                        ...
                        // If the user requested it, start or restart the openECA service
                        if (m_serviceStartCheckBox.IsChecked.Value)
                        {
                            try
                            {
#if DEBUG
                                Process.Start("openECA.exe");
#else
                                m_openECAServiceController.Start();
#endif
                            }
                            catch
                            {
                                MessageBox.Show("The configuration utility was unable to start openECA service, you will need to manually start the service.", "Cannot Start Windows Service", MessageBoxButton.OK, MessageBoxImage.Information);
                            }
                        }

                        // If the user requested it, start the openECA Manager.
                        if (m_managerStartCheckBox.IsChecked.Value)
                        {
                            Process.Start("openECAManager.exe");
                        }
                    }
                    finally
                    {
                        if (m_openECAServiceController != null)
                            m_openECAServiceController.Close();
                    }
                }
        ...
        public Dictionary<string, object> State
        {
            ...
            set
            {
                ...
                if (Convert.ToBoolean(m_state["restarting"]))
                    m_serviceStartCheckBox.Content = "Restart the openECA";
            }
        }
        ...
        // Initializes the openECA service controller.
        private void InitializeopenECAServiceController()
        {
            ServiceController[] services = ServiceController.GetServices();
            m_openECAServiceController = services.SingleOrDefault(svc => string.Compare(svc.ServiceName, "openECA", true) == 0);
        }

        // Initializes the state of the openECA service checkbox.
        private void InitializeServiceCheckboxState()
        {
#if DEBUG
            bool serviceInstalled = File.Exists("openECA.exe");
#else
            bool serviceInstalled = m_openECAServiceController != null;
#endif
            m_serviceStartCheckBox.IsChecked = serviceInstalled;
            m_serviceStartCheckBox.IsEnabled = serviceInstalled;
        }

        // Initializes the state of the openECA Manager checkbox.
        private void InitializeManagerCheckboxState()
        {
            bool managerInstalled = File.Exists("openECAManager.exe");
        ...
        {
            string configFileName = Path.Combine(Directory.GetCurrentDirectory(), "openECA.exe.config");
            ...
            {
                string archiveBinding = string.Format(GrafanaArchiveBinding, "stat", "http.rest://localhost:6362/api/grafana");
            ...
            if (setupHistorian)
            {
                ...
                {
                    string archiveBinding = string.Format(GrafanaArchiveBinding, historianAcronym, "http.rest://localhost:6462/api/grafana");
        ...
        {
            string configFile = Directory.GetCurrentDirectory() + "\\openECA.exe.config";
        ...
        private IDbConnection OpenNewConnection()
        {
            ...
            try
            {
                ...
                else
                {
                    ...
                    dataProviderString = "AssemblyName={System.Data.SQLite, Version=1.0.108.0, Culture=neutral, PublicKeyToken=db937bc2d44ff139}; ConnectionType=System.Data.SQLite.SQLiteConnection; AdapterType=System.Data.SQLite.SQLiteDataAdapter";
                }

...
namespace ConfigurationSetupUtility.Screens
{
    ...
    public partial class ApplyConfigurationChangesScreen : IScreen
    {
        ...
        private void InitializeState()
        {
            ...
            m_openECAManagerLocalCheckBox.IsEnabled = managerOptionsEnabled;
            m_openECAManagerWebCheckBox.IsEnabled = webManagerOptionEnabled;

            // If the options are disabled, they must also be unchecked.
            if (!managerOptionsEnabled)
                m_openECAManagerLocalCheckBox.IsChecked = false;

            if (!webManagerOptionEnabled)
                m_openECAManagerWebCheckBox.IsChecked = false;

            m_setupHistorianCheckBox.IsChecked = false;

            // Set up the state object with the proper initial values.
            m_state["applyChangesToService"] = m_openECAServiceCheckBox.IsChecked.GetValueOrDefault();
            m_state["applyChangesToLocalManager"] = m_openECAManagerLocalCheckBox.IsChecked.GetValueOrDefault();
            m_state["applyChangesToWebManager"] = m_openECAManagerWebCheckBox.IsChecked.GetValueOrDefault();
        ...
        // Occurs when the user chooses to apply changes to the openECA service.
        private void openECAServiceCheckBox_Checked(object sender, RoutedEventArgs e)
        {
            if (m_state != null)
                m_state["applyChangesToService"] = true;
        }

        // Occurs when the user chooses to not apply changes to the openECA service.
        private void openECAServiceCheckBox_Unchecked(object sender, RoutedEventArgs e)
        {
            if (m_state != null)
                m_state["applyChangesToService"] = false;
        }

        // Occurs when the user chooses to changes to the local openECA Manager application.
        private void openECAManagerLocalCheckBox_Checked(object sender, RoutedEventArgs e)
        {
            if (m_state != null)
                m_state["applyChangesToLocalManager"] = true;
        }

        // Occurs when the user chooses to not apply changes to the local openECA Manager application.
        private void openECAManagerLocalCheckBox_Unchecked(object sender, RoutedEventArgs e)
        {
            if (m_state != null)
                m_state["applyChangesToLocalManager"] = false;
        }

        // Occurs when the user chooses to apply changes to the openECA Manager web application.
        private void openECAManagerWebCheckBox_Checked(object sender, RoutedEventArgs e)
        {
            if (m_state != null)
                m_state["applyChangesToWebManager"] = true;
        }

        // Occurs when the user chooses to not apply changes to the openECA Manager web application.
        private void openECAManagerWebCheckBox_Unchecked(object sender, RoutedEventArgs e)
        {
            if (m_state != null)

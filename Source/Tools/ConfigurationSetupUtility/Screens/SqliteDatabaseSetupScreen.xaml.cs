    ...
    public partial class SqliteDatabaseSetupScreen : UserControl, IScreen
    {
        ...
        private void InitializeState()
        {
            ...
            try
            {
                ...
                m_sqliteDatabaseFilePathTextBox.Text = Path.Combine(sqliteDatabaseFilePath, migrate ? "openECA" + App.DatabaseVersionSuffix + ".db" : "openECA.db");
            }
            catch
            {
                m_sqliteDatabaseFilePathTextBox.Text = migrate ? "openECA" + App.DatabaseVersionSuffix + ".db" : "openECA.db";
            }
            ...
            if (existing && !migrate)
            {
                serviceConfig = XDocument.Load(FilePath.GetAbsolutePath("openECA.exe.config"));

...
namespace ConfigurationSetupUtility
{
    ...
    public class SqlServerSetup
    {
        ...
        public void GrantDatabaseAccess(string loginName)
        {
            ...
            ExecuteStatement("IF DATABASE_PRINCIPAL_ID('openECAAdminRole') IS NULL CREATE ROLE [openECAAdminRole] AUTHORIZATION [dbo]");
            ExecuteStatement(string.Format("IF DATABASE_PRINCIPAL_ID('{0}') IS NOT NULL AND DATABASE_PRINCIPAL_ID('openECAAdminRole') IS NOT NULL EXEC sp_addrolemember N'openECAAdminRole', N'{0}'", loginName));
            ExecuteStatement(string.Format("IF DATABASE_PRINCIPAL_ID('openECAAdminRole') IS NOT NULL EXEC sp_addrolemember N'db_datareader', N'openECAAdminRole'"));
            ExecuteStatement(string.Format("IF DATABASE_PRINCIPAL_ID('openECAAdminRole') IS NOT NULL EXEC sp_addrolemember N'db_datawriter', N'openECAAdminRole'"));
        }
                    ...
                    while ((object)line != null)
                    {
                        ...
                        else
                        {
                            if (trimLine.StartsWith("CREATE DATABASE") || trimLine.StartsWith("ALTER DATABASE") || trimLine.StartsWith("USE"))
                                line = line.Replace("openECA", DatabaseName);

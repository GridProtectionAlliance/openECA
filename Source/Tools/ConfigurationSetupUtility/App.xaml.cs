...
namespace ConfigurationSetupUtility
{
    ...
    public partial class App : Application
    {
        ...
        public const string ApplicationExe = "openECA.exe";
        public const string ApplicationConfig = "openECA.exe.config";
        public const string Manager = "openECAManager";
        public const string ManagerExe = "openECAManager.exe";
        public const string ManagerConfig = "openECAManager.exe.config";
        public const string BaseSqliteConfig = "openECA.db";
        public readonly static string SqliteConfigv2 = "openECA" + DatabaseVersionSuffix + ".db";
        public const string SqliteSampleData = "openECA-SampleDataSet.db";
        public const string SqliteInitialData = "openECA-InitialDataSet.db";
        ...
        public App()
        {
            ...
            // Attempt to create an event log source for the openECA Manager for authentication logging. This needs to be done
            // here since the CSU runs with administrative privileges and the openECA Manager normally does not; also there is
            // a short system delay that exists before you can write to a new event log source after it is first created.
            try
            {
                string applicationName = "openECA";

                // Create the event log source based on defined application name for openECA if it does not already exist
                if (!EventLog.SourceExists(applicationName))
                    EventLog.CreateEventSource(applicationName, "Application");

                applicationName = "openECA Manager";

                // Create the event log source based on defined application name for openECA Manager if it does not already exist
                if (!EventLog.SourceExists(applicationName))
                    EventLog.CreateEventSource(applicationName, "Application");
            }
            catch (Exception ex)
            {
                m_errorLogger.Log(new InvalidOperationException(string.Format("Warning: failed to create or validate the event log source for the openECA Manager: {0}", ex.Message), ex), false);
            }
        }

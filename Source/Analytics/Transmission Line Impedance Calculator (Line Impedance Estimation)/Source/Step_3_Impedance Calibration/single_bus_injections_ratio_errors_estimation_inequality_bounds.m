function [KI_injection_set]=single_bus_injections_ratio_errors_estimation_inequality_bounds(current_bus_number,injection_line_number_set,...
...
upper_bound = [];
lower_bound = [];
for idx3 = 1:current_leftside_columns
    left_side(:,2*idx3-1) = real(I_leftside_set(:,idx3));
    left_side(:,2*idx3) = imag(I_leftside_set(:,idx3));
    
    upper_bound(2*idx3-1,1) = 1.0526;
    upper_bound(2*idx3,1) = 0.1005;
    
    lower_bound(2*idx3-1,1) = 0.9452;
    lower_bound(2*idx3,1) = -0.1005;
end

right_side = real(I_rightside_set(:,1));
...
for NN=1:30
    ...
    left_side_sample = left_side(jk,:);
    right_side_sample = right_side(jk,:);
    
    options = optimoptions('lsqlin','Algorithm','trust-region-reflective','Display','iter','TolFun',1e-10);
    [T, x_samples(:,NN)] = evalc('lsqlin(left_side_sample,right_side_sample,[],[],[],[],lower_bound,upper_bound,[],options);');
end

x = sum(x_samples,2)/NN;

estimation_results = [];
for idx4 = 1:(size(x)/2)
    estimation_results = [estimation_results; x(2*idx4-1,1)-1i*x(2*idx4,1)];
end
